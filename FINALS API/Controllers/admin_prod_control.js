...
const updateProduct = (req, res, next) => {
  ...
  if (!ProdName && !ProdDescription && !Price) {
    ...
    let query = `SELECT prod_ID, Prod_Name, Prod_Desc, price FROM products WHERE prod_ID=${prodID}`;
...
const deleteProduct = (req, res, next) => {
  ...
  if (!id) {
    ...
    let query = `SELECT prod_ID FROM products WHERE prod_ID=${id}`;
    ...
    database.db.query(query, (err, rows, result) => {
      if (err) {
        ...
        if (rows.length > 0) {
          let deleteQuery = `DELETE FROM products WHERE prod_ID=${id}`;
